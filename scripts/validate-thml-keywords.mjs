/**
 * Validate Q|A keywords for banks/thml.js (keywordsRulesVN.md)
 * Usage: node scripts/validate-thml-keywords.mjs
 */
import fs from "fs";
...
function validateQuestion(q) {
    ...
    for (const frag of fragments) {
        ...
        if (countWords(frag) > MAX_FRAG_WORDS) warnings.push(`long A fragment: "${frag}"`);
    }
...
function main() {
    ...
    console.log(`Validated ${bank.length} THML questions (Q|A format)\n`);
    if (failed.length) {
    ...
    if (warned.length) {
        ...
        warned.slice(0, 15).forEach((r) => console.log(`  #${r.id}: ${r.warnings.join("; ")}`));
